...
public class SerialReader implements DelimiterSerialCallBack{
	int baudrate = 9600; // this does not matter, Teensy uses USB protocol
	...
	public SerialReader(PApplet applet, BufferCallBack callback, int bufferSize) {
		this.callback = callback;
		this.bufferSize = bufferSize;
		this.dataBuffer = new float[2][bufferSize*2];
	...
	public void onBuffer(byte[] buffer, int readlen) {
		if(readlen > 0 && convertBuffer(buffer, readlen, dataBuffer[curBuffer])) {
			
			// ADC stream
			if(readlen == dataBuffer[curBuffer].length *2){
				callback.onBuffer(dataBuffer[curBuffer]);
	...
	private static boolean convertBuffer(byte[] buffer, int readlen, float[] floatBuffer) {
		if(readlen != floatBuffer.length * 2 && readlen != 3) {
			System.out.println("Got bad buffer with length: " + readlen);
			return false;
		}
